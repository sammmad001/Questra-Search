...
Questra-Search 是一个基于 MiroThinker 深度推理模型的研究助手代理，支持多步推理、工具调用（搜索、代码执行等）和流式 SSE 输出。每次对话生成的研究成果（含推理过程、工具调用链、最终结论）目前仅存储在 Questra-Search 本地 SQLite 数据库中，形成了**数据孤岛**。

个人知识库（KB）是一个 Graph-First 架构的知识管理系统（Neo4j + FastAPI），已实现完整的 4 步知识入库管道（预处理→分析→图谱处理→渲染），但缺少自动化的多渠道知识汇聚能力。

### 1.2 核心问题

1. **知识断层**：Questra-Search 深度研究的成果无法自动回流到个人知识库
...
建立 Questra-Search → 个人知识库的**自动化闭环**：每次 Questra-Search 对话完成后，自动将助手回复（含推理过程、工具调用链）通过标准化 API 导入到个人知识库，经过语义分析、实体提取、图谱构建后融入知识网络。
...
- **低侵入性**：Questra-Search 侧代码变更最小化
- **高容错性**：集成失败不影响 Questra-Search 主聊天功能
- **可观测性**：关键操作有日志记录
...
│  │ (每5分钟重试)     │         POST /api/ingest/questra-search         │
│  └──────────────────┘                                           │
└──────────────────────────────────────────────────────────────────┘
                              │
                              ▼
...
│  │ questra-search │  │  /transform      │  │  →render         │ │
│  └────────────────┘  └──────────────────┘  └──────────────────┘ │
└──────────────────────────────────────────────────────────────────┘
```
...
用户发消息 → chat.py 查询 session title/model
           ...
           → asyncio.ensure_future(_post_to_kb())  // fire-and-forget
               → POST /api/ingest/questra-search
               → KB 侧 Questra-SearchAdapter 处理
               → 4 步管道 → Neo4j
```
...
KbRetrySender 每 5 分钟:
  ...
  → 逐条 POST /api/ingest/questra-search
...
- **只传不读**：Questra-Search 仅 POST 数据到 KB，不读取 KB 内容
...
```
questra-search-kb-integration/
├── .env.example                  ← 覆盖 Questra-Search 根目录
├── 部署步骤.md                    ← 部署指南
└── app/
...
# 返回示例
{"total_ingested": 15, "total_pending": 0, "total_failed": 2, "by_source": {"questra-search": 15}}
```

**3. 数据库验证**（Questra-Search 侧）：
```sql
-- 查看哪些消息尚未发送
...
**Q: 如果 KB 和 Questra-Search 不在同一台机器？**
...
### KB 侧接口：`POST /api/ingest/questra-search`
